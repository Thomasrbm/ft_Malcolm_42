...
from scapy.arch.linux import get_last_packet_timestamp, SIOCGinterface_idx
...
class sockaddr_can(ctypes.Structure):
    ...
    _fields_ = [("can_family", ctypes.c_uint16),
                ("can_interface_idx", ctypes.c_int),
                ("can_addr", addr_info)]
...
class ifreq(ctypes.Structure):
    ...
    _fields_ = [("ifr_name", ctypes.c_char * 16),
                ("ifr_interface_idx", ctypes.c_int)]
...
class ISOTPNativeSocket(SuperSocket):
    ...
    def __get_sock_ifreq(self, sock, iface):
        ...
        ret = LIBC.ioctl(socket_id, SIOCGinterface_idx, ctypes.byref(ifr))
