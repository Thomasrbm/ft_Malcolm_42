...
def _read_routes_c_v1():
    ...
    for route in GetIpForwardTable():
        interface_idx = route['Forwardinterface_idx']
        ...
        # Build route
        try:
            iface = cast(NetworkInterface_Win, dev_from_index(interface_idx))
...
def _read_routes_c(ipv6=False):  # noqa: F811
    ...
    for route in GetIpForwardTable2(af):
        # Extract data
        interface_idx = route['InterfaceIndex']
        ...
        # Build route
        try:
            iface = dev_from_index(interface_idx)
            if not iface.ip or iface.ip == "0.0.0.0":
                continue
        except ValueError:
            continue
